...
print(f"{'Date':<12} | {'Gap %':<7} | {'Target%':<7} | {'Hit?':<5} | {'Act P&L ($)':<12} | {'Act (%)':<8} | {'Sim P&L ($)':<12} | {'Sim (%)':<8} | {'Friction':<10}")
print("-" * 105)

for i, date_str in enumerate(dates):
    eq_array = daily_data[date_str]["equity"]
    
    # Actuals
    if i == 0:
        actual_prev_eq = 30000.0
    else:
        actual_prev_eq = official_baselines.get(dates[i-1], eq_array[0])
        if actual_prev_eq < 20000: actual_prev_eq = 30000.0
        
    actual_open_eq = eq_array[0]
    actual_close_eq = eq_array[-1]
    
    actual_gap_pct = (actual_open_eq / actual_prev_eq) - 1 if actual_prev_eq > 0 else 0
    actual_day_pnl_val = actual_close_eq - actual_prev_eq
    actual_day_pnl_pct = (actual_day_pnl_val / actual_prev_eq) * 100
    ...
    if held_cash_overnight:
        ...
        # Re-entry friction: 3x leverage * 0.1% spread = 0.3% of equity
        re_entry_friction = sim_open_eq * 0.003
    ...
    # Target calculate using exact requested formula
    target_pct = 1.75
    ...
    for bar_eq in eq_array:
        ...
        if sim_current_pnl_pct >= target_pct:
            ...
            # Liquidation friction: 3x leverage * 0.1% spread = 0.3% of equity
            liq_friction = sim_close_eq * 0.003
    ...
    act_pnl_str = f"${actual_day_pnl_val:+.2f}"
    sim_pnl_str = f"${sim_day_pnl_val:+.2f}"
    fric_str = f"-${daily_friction_cost:.2f}"
    
    print(f"{date_str:<12} | {sim_gap_pct:>6.2f}% | {target_pct:>6.2f}% | {hit_str:<5} | {act_pnl_str:<12} | {actual_day_pnl_pct:>+6.2f}% | {sim_pnl_str:<12} | {sim_day_pnl_pct:>+6.2f}% | {fric_str:<10}")

print("-" * 105)
print(f"ACTUAL Current Equity: ${actual_close_eq:,.2f}")
print(f"SIMULATED Current Equity: ${sim_eq:,.2f}")
print(f"NET DIFFERENCE: ${(sim_eq - actual_close_eq):,.2f}")
